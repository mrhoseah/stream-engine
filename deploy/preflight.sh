...
AMS_ENFORCE_HTTPS="${AMS_ENFORCE_HTTPS:-true}"
...
if [[ "$AMS_ENFORCE_HTTPS" == "true" && -n "$AMS_BASE_URL" && "$AMS_BASE_URL" != https://* ]]; then
  fatal "AMS_ENFORCE_HTTPS=true but AMS_BASE_URL is not https:// (${AMS_BASE_URL})"
fi
